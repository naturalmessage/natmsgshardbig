...
###############################################################################
#
# The Online privage sign key is for the Natural Message server verification
# process.  The value is set by the calling server usnig data from the
# option file.
  ...
  This function will also show Python error information and traceback.
    ...
    try:
        ...
        if simulate_err_nbr and err_nbr == simulate_err_nbr:
            ...
            raise RuntimeError(
                'Simulated error for testing purposes. Reference '
                + str(err_nbr))
...
def log_err(
    ...
    if show_python_err:
        ...
        if sys_msg is None:
            ...
            err_msg = val + '...  PYTHON ERR MSG from the shard server: ' \
                + sys_msg
    else:
        err_msg = val
...
def shard_connect(conn_str):
    ...
    try:
        ...
        out.update(
            {'Error': 'There was a data type error when trying '
        ...
        out.update({'Error-detail': repr(sys.exc_info())})
        out.update(
            {'Error': 'Some other type of error during conneciton '
...
###############################################################################
        ...
        out_msg.update(
            {'Error': 'There was a data integrity problem, such as '
            + 'duplicate key in the PostgreSQL database.'})
        rc = 13001

    except(psycopg2.DataError):
        out_msg.update(
            {'Error': 'There was a data problem, such as value too '
            + 'big for the insert target field.'})
        rc = 13020

    except(psycopg2.ProgrammingError):
        # Check the databse log file for more info:
        #   * /var/log/httpd/error_log, or
        #   * /var/lib/pgsql/data/pg_log on the server
...
def shard_sql_select(cur, cmd, binary_output=False):
    ...
    try:
        ...
        out_msg.update({'Error-detail': repr(sys.exc_info())})
        out_msg.update(
            {'Error': 'There was a data problem, such as value too big '
            + 'for the insert target field--probably not needed for a '
            + 'read operation.'})
        rc = 13070
    except(TypeError):
        out_msg.update({'Error-detail': repr(sys.exc_info())})
        out_msg.update({'Error': 'There was a TYPE error, possibly in the '
            + 'command sent to the cur.excute() function.'})
        rc = 13080
    except:
        out_msg.update({'Error-detail': repr(sys.exc_info())})
...
def validate_id_chars(id_chunk):
    ...
    if (len(tmp2) != 0):
        ...
        out_msg.update(
            {'Error': 'Illegal characters (' + tmp2 + ') in the random '
            + 'part of the id: ' + id_chunk})
        rc = 13090
        return((rc, out_msg))
...
def verify_id_format(id, expected_prefix, version=1):
    ...
    if expected_prefix is None:
        out_msg.update(
            {'Error': 'There was no expected_prefix passed to '
            + 'verify_id_format.'})
        rc = 13100
        return((rc, out_msg))
    ...
    if id is None:
        out_msg.update(
            {'Error': 'There was no ID passed to verify_id_format.'})
        rc = 13110
        return((rc, out_msg))
    ...
    if (expected_prefix in ('SID', 'SMD', 'MIX')):
        ...
        if (len(id) == 35):
            ...
            out_msg.update(
                {'Error': 'Length of a shard ID must be 35. Observed '
                + 'length was: ' + str(len(id))})
            #raise Exception('Illegal ID length')
            rc = 13150
            return((rc, out_msg))
        ...
        if (len(id) == 141):
            ...
            if (id[0:3] == 'PRV'):
                if (test_yyyymmdd != 40010101):
                    ...
                    out_msg.update(
                        {'Error': 'The expiration date for PRV IDs should '
                        + 'be 40010101, but this one is: '
                        + str(test_yyyymmdd)})
                    rc = 13170
                    return((rc, out_msg))
            else:
                if (test_yyyymmdd > ID_CUTOFF_YYYYMMDD):
                    out_msg.update(
                        {'Error': 'The expire date for this account '
                ...
                if (test_mm > 12):
                    out_msg.update(
                        {'Error': 'invalid month for ID: '
                        + str(test_mm)})
                    rc = 13200
                    return((rc, out_msg))

                if (test_mm > 31):
                    out_msg.update({'Error': 'invalid day for ID: '
                        + str(test_dd)})
                    rc = 13210
                    return((rc, out_msg))
            ...
            out_msg.update(
                {'Error': 'Length of ID must be 85 (3-letter code, 2-byte '
                + 'reserved, 8-byte date, 40 bytes server fingerprint, 32 '
                + 'bytes random).  '
...
def gpg_enc_str(
    ...
    send it to the specified output file.  If the user specified '-' as the
    filename, the output will go to STDOUT.
    """
    # This version expects a io.StrinigIO() object. To read bytes objects,
    # make a modified copy of this and remove the bytes command inside
    # communicate().
    ...
    if not clobber:
        if os.path.isfile(output_fname):
            try:
                ...
                out.update({'Error': 'Could not rename existing file: '
                    + output_fname})
                rc = 13217
                return((rc, out))
    else:
        ...
        if os.path.isfile(output_fname):
            try:
                ...
                out.update({'gpg_enc_strError': sys.exc_info()[1]})
                out.update(
                    {'Error': 'Output file exists and I can not remove it: '
                    + output_fname})
                rc = 13220
                return((rc, out))
    ...
    try:
        ...
        gpg_process = subprocess.Popen(
            ['gpg', '-e', '-a', '--default-key', default_gpg_id,
    ...
    if (gpg_process.returncode == 2):
        out.update(
            {'Error': 'GPG failed with error code 2. You might need to sign '
            + 'the dest public key using the '
            + 'gpg --default-key YOUR_PRV_KEY --sign-key DEST_PUB_KEY '
            + 'and be sure that you set the --default-key so that you know '
            + 'that the default private key used here is the same one used '
            + 'to sign the public key.'})
        #out.update({'gpg_enc_strError': sys.exc_info()[1]})
...
def gpg_decrypt(
    ...
    that this server holds in its default gpg homedir.  The decrypted file
    will be written to the output filename, or to STDOUT if the filename
    is set to '-'.
    """
    out = {}
    rc = 0
    ...
    if not overwrite:
        ...
        if os.path.isfile(output_fname):
            try:
                ...
                msg = 'Overwrite option was wet to True, but could not ' \
                    + 'remove the existing file: ' + output_fname
                rc = 151
                out.update(log_err(rc, msg))
                return((rc, out))
    ...
    process_list = [
            'gpg', '-d', '--use-agent', '--output', output_fname,
             '--batch', input_fname
        ]
    ...
    if (gpg_process.returncode == 2):
        # Not sure if this applies to decrytping
        # it does apply to encryption.
        msg = 'GPG failed with error code 2. You might need to sign the ' \
            + 'dest public key using the ' \
            + 'gpg --default-key YOUR_PRV_KEY --sign-key DEST_PUB_KEY ' \
            + 'and be sure that you set the --default-key so that ' \
            + 'you know that the default private key used here is ' \
            + 'the same one used to sign the public key. From gpg: ' + gpg_out
        rc = 2
        out.update(log_err(rc, msg))
        return((rc, out))
    elif (gpg_process.returncode != 0):
...
class RNCrypt_zero(RNCryptor.RNCryptor):
    ...
    def post_decrypt_data(self, data, decrypt_to_str=False):
        """Removes useless symbols which appear over padding
        for AES (PKCS#7).
        """
    ...
    def decrypt(self, data, password, decrypt_to_str=False):
        ...
        return self.post_decrypt_data(
            decrypted_data, decrypt_to_str=decrypt_to_str)


    ## If I remove the thing that forces RNCryptor decryptions to be str,
    ## then I would have to wrap the shard_read results in another layer
    ## of base 64.
...
def __sign_data(nonce=None):
    ...
    if ONLINE_PRV_SIGN_KEY_FNAME == '':
        raise RuntimeError(
            '457394: In __sign_data, the ONLINE_PRV_SIGN_KEY_FNAME is '
            + 'missing. This value should be set from the shard '
            + 'server during initialzation.')
    ...
    try:
        ...
        print('++ Signing of nonce failed.. could not run '
            + 'the nm_sign program.')
        rc = 7900
    ...
    if (sig == b''):
        print('Error. Signing of nonce failed. PRV key is '
            + ONLINE_PRV_SIGN_KEY_FNAME)
        rc = 8000
        msg = 'Could not sign the file.  The nm_sign program gave this ' \
            + 'error message <' + err.decode('utf-8') + '>.' \
            + ' (ignore any error saying that secure memory ' \
            + 'was alrady allocated.)'
        out.update(log_err(rc, msg))
        return({'sign_data': out})
...
def sign_nonce(nonce=None):
    ...
    if nonce is not None:
        ...
        try:
            sig_b64_dict = sig_dict['sign_data']
