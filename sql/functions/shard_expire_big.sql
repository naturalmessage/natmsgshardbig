-- shard_expire_big.sql contains function shardsvr.shard_expire
...
--
-- Function:
-- 1) This function puts records into shardsvr.shard_kill_pending
...
-- ----------------------------------------
-- ----------------------------------------
DROP FUNCTION IF EXISTS shardsvr.shard_big_expire();

CREATE FUNCTION shardsvr.shard_big_expire() RETURNS int AS $$
DECLARE
rows_expired int;
BEGIN
	...
	INSERT INTO shardsvr.shard_big_kill_pending(big_shard_id, big_shard_pkid)
	select big_shard_id, big_shard_pkid from shardsvr.big_shards
	WHERE 
		burned = false
		and expired = false
    ...
    expire_on_date = null
	WHERE 
		big_shard_pkid in (select big_shard_pkid from shardsvr.shard_big_kill_pending);
